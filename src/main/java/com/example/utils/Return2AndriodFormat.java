...
public class Return2AndriodFormat {
	 ...
	 * 
	 * @param r 状态常量
	 * @param data 封装了数据的JSONObject
	 * @return
	 */
	public static JSONObject getResult(ResponseModel r,JSONObject data){
		if(data==null){
			data=new JSONObject();
		}
		JSONObject baseJSONOBject = new JSONObject();
		baseJSONOBject.put("status", r.getStatus());
		baseJSONOBject.put("msg", r.getMsg());
		baseJSONOBject.put("data", data);
		return baseJSONOBject;
	}
	public static JSONObject getCookResult(ResponseModel r,JSONObject data){
		if(data==null){
			data=new JSONObject();
		}
		JSONObject baseJSONOBject = new JSONObject();
		baseJSONOBject.put("resultcode", r.getStatus());
		baseJSONOBject.put("reason", r.getMsg());
		baseJSONOBject.put("error_code", r.getError_code());
		baseJSONOBject.put("result", data);
		return baseJSONOBject;
	}
	/**
	 ...
	 * @return
	 */
	public static JSONObject getResultArray(ResponseModel r,JSONArray data){
		if(data==null){
			data=new JSONArray();
		}
		JSONObject baseJSONOBject = new JSONObject();
